...
docket shows every open GitHub pull request and issue the viewer is involved in, with a local archive to hide any of them, as a terminal UI (`docket`), a localhost web page (`docket web`) and a one-shot print (`docket dump`). Go, one binary, auth borrowed from the `gh` CLI.
...
internal/fetch/      discovery, detail and archived-item queries; API responses into model.PR and model.Issue
internal/model/      pure rules: activity, what's left, my side, sections, the archive filter
...
- GraphQL: an error without data is a failed poll, never an empty list; partial data becomes warnings. Detail goes 10 PRs a request, since 20 drew 502s from GitHub's query time limit; issues are lighter and go 25.
...
go run ./tools/screenshots tui 150 32 > tui.ansi     # extra args are keys to press first, e.g. tab or s
freeze -l ansi --window --font.family "MesloLGM Nerd Font Mono" --font.size 4.895 --line-height 1.216 -o docs/img/tui.png tui.ansi
go run ./tools/screenshots web                        # demo page on 127.0.0.1:7799
```

Name the font by family: with `--font.file`, the SVG renderer `freeze` hands off to draws box characters at the wrong width. That size and line height turn a 150×32 frame into 2000×960. SF Mono lacks `◆`. Take the web shots with a headless browser at 2x (a 1000×683 viewport), in dark and light.
